...
function filterStudentsByGrade(students, grade) {
  ...
  const studentsA = students.filter((student) => student.grade === grade);
  studentsA.forEach((student) => console.log(student));
}
...
function traceBackward(n) {
  ...
  if (n > 1) {
    traceBackward(n - 1);
    if (n > 1) traceBackward(n - 1);
    else console.trace(n);
  }
}
...
function validateUserInput(userInput) {
  ...
  if (userInput.username.length > 0) {
    if (userInput.password.length > 0) {
      ...
      console.exception("Помилка: пароль відсутній!");
    }
  } else {
    console.exception("Помилка: ім'я користувача відсутнє!");
  }
}
...
function calculateTotalPrice(products) {
  ...
  const timer = "calculateTotalPrice";
  console.time(timer);
  let total = 0;
...
function countVowelsAndConsonants(word) {
  ...
  for (const letter of word) {
    //letter.toLowerCase();
    if (vowels.includes(letter.toLowerCase())) console.count("Голосна");
    else console.count("Приголосна");
  }
}
